...
export const fetchToken = async (request?: Request) => {
  ...
  const sapiResponse = await fetch(`${baseUrl}/token`, {
    headers: headersToRelay,
  });
  if (!sapiResponse.ok) {
    throw new Error(
      `Failed to fetch token: ${sapiResponse.status} ${sapiResponse.statusText}`,
    );
  }
...
const extractCookiesFromRequest = (request?: Request) => {
  ...
  const cookieHeader =
    request && request.headers && request.headers.get('Cookie');
  if (cookieHeader) {
